''' <summary>
''' D3D_InvalidationRouter 是 GPU 控件的非渲染失效入口。
''' 它只规范脏区并交给外到内调度器；真正的 GPU 提交由 V5 presentation 或 Paint bridge 完成。
''' </summary>
Friend NotInheritable Class D3D_InvalidationRouter
    ...
    ''' <summary>
    ''' 控件状态变化时调用。这里不立即绘制，只合并到统一的外到内失效队列。
    ''' </summary>
    Public Shared Sub RequestRender(control As Control, dirtyRect As Rectangle)
        ...
        ' 在父级表面准备好之前先渲染，背景映射因此可能采样到旧表面。
        ' 调度器只负责合并和排序失效；OnPaint 到达后由 V5 bridge 提交 GPU 帧。
